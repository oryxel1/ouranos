...
public class GlobalWorldTranslator extends ProtocolToProtocol {
    ...
    protected void registerProtocol() {
        this.registerClientbound(UpdateBlockPacket.class, wrapped -> {
            ...
            packet.setDefinition(new SimpleBlockDefinition(TypeConverter.translateBlockRuntimeId(wrapped.getInput(), wrapped.getOutput(), packet.getDefinition().getRuntimeId())));
        });
        ...
        this.registerClientbound(LevelSoundEventPacket.class, wrapped -> {
            ...
            if (sound == SoundEvent.PLACE || sound == SoundEvent.BREAK_BLOCK || sound == SoundEvent.ITEM_USE_ON) {
                packet.setExtraData(TypeConverter.translateBlockRuntimeId(wrapped.getInput(), wrapped.getOutput(), packet.getExtraData()));
            }
        });
        ...
        this.registerClientbound(EntityEventPacket.class, wrapped -> {
            ...
            if (packet.getType() == EntityEventType.EATING_ITEM) {
                ...
                int[] newItem = TypeConverter.translateItemRuntimeId(wrapped.getInput(), wrapped.getOutput(), data >> 16, data & 0xFFFF);
        ...
        this.registerClientbound(AddEntityPacket.class, wrapped -> {
            ...
            if (runtimeId != null) {
                data.put(EntityDataTypes.VARIANT, TypeConverter.translateBlockRuntimeId(wrapped.getInput(), wrapped.getOutput(), runtimeId));
            }
        });
        ...
        this.registerClientbound(UpdateSubChunkBlocksPacket.class, wrapped -> {
            ...
            for (final BlockChangeEntry entry : packet.getExtraBlocks()) {
                newExtraBlocks.add(new BlockChangeEntry(entry.getPosition(), TypeConverter.translateBlockDefinition(wrapped.getInput(), wrapped.getOutput(), entry.getDefinition()), entry.getUpdateFlags(), entry.getMessageEntityId(), entry.getMessageType()));
            }
            ...
            for (final BlockChangeEntry entry : packet.getStandardBlocks()) {
                newStandardBlock.add(new BlockChangeEntry(entry.getPosition(), TypeConverter.translateBlockDefinition(wrapped.getInput(), wrapped.getOutput(), entry.getDefinition()), entry.getUpdateFlags(), entry.getMessageEntityId(), entry.getMessageType()));
            }
        ...
        this.registerClientbound(LevelChunkPacket.class, wrapped -> {
            ...
            try {
                var newSubChunkCount = TypeConverter.rewriteFullChunk(wrapped.getInput(), wrapped.getOutput(), from, to, packet.getDimension(), packet.getSubChunksLength());
        ...
        this.registerClientbound(SubChunkPacket.class, wrapped -> {
            ...
            for (final SubChunkData subChunk : packet.getSubChunks()) {
                if (subChunk.getData().readableBytes() > 0) {
                    ...
                    try {
                        TypeConverter.rewriteSubChunk(wrapped.getInput(), wrapped.getOutput(), from, to);
